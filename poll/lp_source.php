...
function ShowTheStuff($item, $smallpic, $bigpic, $itemvoted, $graph_width, $graph_height) {
	...
	$stepstr=$stepstr.'<table align=center border=0><tr>';
	while ($in<$hector) {
		$timesred=(int)((($itemvoted[$in]/$totalvotes))*$graph_width);
		$stepstr=$stepstr.'<td><table align=center border=0><tr><td>&nbsp</td><td><font size="2">'.stripslashes($item[$in]).'</td></tr>';
		$stepstr=$stepstr.'<tr><td align="center"><font size="2">'.(int)(($itemvoted[$in]/$totalvotes)*100).'%<br><img width='.$graph_height.' height='.($graph_width-$timesred).' src="lp_0.gif"><br><img width='.$graph_height.' height='.$timesred.' src="lp_1.gif"></td>';
		...
		if ((($in-$ncols+1) % $ncols) == 0)  {
			$stepstr=$stepstr.'</tr></table><table align=center border=0><tr>';
		}
		$in++;
	}
...
if ($votingstep==1) {
	...
	$totalvotes=SumArray($itemvoted);
	$in=0;
	$step1str=$step1str.'<table align=center border=0><tr>';
	$datop=count($item);
	while($in<$datop){
	    $step1str=$step1str.'<td class=pow><table align=center border=0><tr>';
		$step1str=$step1str.'<td><input type="radio" name="radios" value="'.$in.'"></td><td><font size="2">'.stripslashes($item[$in]).'</td></tr>';
		//$step1str=$step1str.'<tr><td align="center"><font size="2">'.(int)(($itemvoted[$in]/$totalvotes)*100).'%<br><img width='.$graph_height.' height='.($graph_width-$timesred).' src="lp_0.gif"><br><img width='.$graph_height.' height='.$timesred.' src="lp_1.gif"></td>';
		...
		if ((($in-$ncols+1) % $ncols) == 0)  {
		$step1str=$step1str.'</tr></table><table align=center border=0><tr>';
		}

		$in++;
	}
	$step1str=$step1str.'</tr></table>';
	$step1str=$step1str.'<br><input class="btn btn-success btn-lg" style="'.$buttonstyle.'" type="Submit" value="'.$vote_str.'"></form>';
}
